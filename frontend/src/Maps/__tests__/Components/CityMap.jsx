...
describe('<CityMap />', () => {
    test('renders correctly', () => {
        let city = {name_std: "Bristol", country: "United Kingdom", lat: 0.0, lng: 0.0 };
        const result = renderer.render(<CityMap city={city}/>);
        expect(result).toMatchSnapshot();
    });

    test("City's lat lng are set as Map child center prop correctly", () => {
        let city = {name_std: faker.address.city(), country: faker.address.country(), lat: faker.address.latitude(),
            lng: faker.address.longitude() };
        let center = [city.lat, city.lng];
